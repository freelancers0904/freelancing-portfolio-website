...
const Services = () => {
  ...
  return (
    <section id="services" className="py-28 md:py-32" style={{ background: '#080808' }}>
      <div ref={sectionRef} className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16">
          {services.map((s, i) => (
            <div key={i} className="scroll-hidden glass-card p-9 relative overflow-hidden">
              {/* Decorative glow */}
              <div className="absolute -top-10 -right-10 w-[120px] h-[120px] rounded-full blur-[30px] pointer-events-none" style={{ background: s.glowColor }} />

              <div className="w-11 h-11 rounded-[10px] flex items-center justify-center" style={{ background: s.tagBg, border: `1px solid ${s.iconBorder}` }}>
                {s.icon}
              </div>
              <div className="mt-5 inline-block px-2.5 py-1 rounded-full text-[10px] font-semibold tracking-[1.5px] font-body" style={{ background: s.tagBg, color: s.iconColor }}>
                {s.tag}
              </div>
              ...
              {s.hasBottomLine && (
                <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: 'linear-gradient(90deg, #6C63FF, #00D4AA)' }} />
              )}
            </div>
          ))}
        </div>
